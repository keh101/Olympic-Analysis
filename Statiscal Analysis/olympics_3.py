...
def plot_bar_PCA(per_var,labels,Sport):
    #plots PCA as bar chart showing which PCs account for the most variance
    plt.bar(x=range(1,len(per_var)+1),height=per_var,tick_label=labels)
    plt.ylabel('Percentage of Explained Variance')
    plt.xlabel('Principal Component')
    plt.title('Scree Plot')
    plt.savefig(r'C:\Users\nikhil\Desktop\ECE143 HW\Olympic-Analysis-master\AnalyzedData\PCA\%s_PCA_Bar.png' %Sport)
    plt.show()

def plot_scatter_PCA(pca_data,listof_indices,labels,per_var,Sport):
    ...
    plt.show()
    fig2.get_figure().savefig(r'C:\Users\nikhil\Desktop\ECE143 HW\Olympic-Analysis-master\AnalyzedData\PCA\%s_PCA_Scatter.png' %Sport)
    
def top_10_PCA(pca,listof_columns,Sport):
    ...
    loading_scores[top_10_years].to_csv(r'C:\Users\nikhil\Desktop\ECE143 HW\Olympic-Analysis-master\AnalyzedData\PCA\%s_top10_PCA.csv' %Sport)
...
def perform_trend_analysis(variable_of_int):
    ...
    #read in data
    df_to = pd.read_csv(r"C:/Users/nikhil/Desktop/ECE143 HW/Olympic-Analysis-master/AnalyzedData/Height_Sport_Year.csv")
    multiindex_height = df_to.set_index(['Sport','Year','Height'])
    
    #Calculate and graph HP data for height
    for sports in listofsports:
        print(sports)
        dict_of_height_nan = prepare_data(multiindex_height,sports[0], nan='yes')
        try:    
            if dict_of_height_nan is not None:
                hp_info_height = calculate_hp_trends(dict_of_height_nan['prepared_data'])
                graph_hp_trends(sports[0],hp_info_height['hp_cycle'],hp_info_height['hp_trend'])
        except ValueError:
...
def perform_PCA_analysis(variable_of_int):
    ...
    for sports in listofsports:
        try:
            print(sports)
            dict_of_height_0 = prepare_data(multiindex_height,sports[0], nan='no')
            if dict_of_height_0 is not None:
                    pca_info = calculate_PCA(dict_of_height_0['prepared_data'])
                    plot_bar_PCA(pca_info['per_var'],pca_info['labels'],sports[0])
                    plot_scatter_PCA(pca_info['pca_data'],dict_of_height_0['listof_indices'],pca_info['labels'],pca_info['per_var'],sports[0])
                    top10_info = top_10_PCA(pca_info['pca'],dict_of_height_0['listof_columns'],sports[0])
                    print(top10_info)
