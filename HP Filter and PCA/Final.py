...
def graph_hp_trends(Sport,hp_cycle,hp_trend,variable_of_int):
    ...
    if not hp_trend.empty:
        ...
        fig1.savefig('./AnalyzedData/%s/Figures/%s_hptrends.png' %(variable_of_int,Sport))
    else:
    ...
    if not hp_cycle.empty:
        ...
        fig2.savefig('./AnalyzedData/%s/Figures/%s_hpcycles.png' %(variable_of_int,Sport))
    else:
...
def plot_bar_PCA(per_var,labels,Sport,variable_of_int):
    ...
    plt.savefig('./AnalyzedData/%s/PCA/%s_PCA_Bar.png' %(variable_of_int,Sport))
    plt.show()
...
def plot_scatter_PCA(pca_data,listof_indices,labels,per_var,Sport, variable_of_int):
    ...
    plt.show()
    fig2.get_figure().savefig('./AnalyzedData/%s/PCA/%s_PCA_Scatter.png' %(variable_of_int,Sport))
...
def top_10_PCA(pca,listof_columns,Sport,variable_of_int):
    ...
    loading_scores[top_10_years].to_csv('./AnalyzedData/%s/PCA/%s_top10_PCA.csv' %(variable_of_int,Sport))
...
def perform_trend_analysis(variable_of_int):
    ...
    with open('./120-years-of-olympic-history-athletes-and-results/sports.csv', 'r') as f:
        reader = csv.reader(f)
        listofsports = list(reader)
    #read in data
    df_to = pd.read_csv("./AnalyzedData/%s_Sport_Year.csv"%variable_of_int)
